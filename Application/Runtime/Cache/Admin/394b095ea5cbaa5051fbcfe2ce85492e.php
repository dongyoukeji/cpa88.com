...
<div class="admin_r_all" style="height: 477px;">
    <div class="admin_bg_all">
        <div class="admin_bg_t">
            <a href="#" id="admin_bg_t_x">广告列表</a>
            <a href="/Admin/Advert/add">广告添加</a>
        </div>
        <div class="admin_bg_b">
            <ul class="admin_bg_b_t">
                <li class="admin_bg4">编号</li>
                <li class="admin_bg4">广告标题</li>
                <li class="admin_bg4">广告图片</li>
                <li class="admin_bg4">广告链接</li>
            ...
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
                        ...
                        <li class="admin_bg4"><img src="<?php echo ($vo["image"]); ?>" alt="" height="30" style="vertical-align: middle;"></li>
                        <li class="admin_bg4"><?php echo ($vo["url"]); ?></li>
                        <li class="admin_bg4"><?php if(($vo["status"]) == "0"): ?>正常<?php else: ?>禁用<?php endif; ?> </li>
                        <li class="admin_bg4">
                            <a href="/Admin/Advert/add?id=<?php echo ($vo["id"]); ?>">编辑</a>
                            <?php if(($vo["status"]) == "0"): ?><a href="/Admin/Advert/status?id=<?php echo ($vo["id"]); ?>&t=1">禁用</a><?php else: ?><a href="/Admin/Advert/status?id=<?php echo ($vo["id"]); ?>&t=0">正常</a><?php endif; ?>
                            <a href="/Admin/Advert/del?id=<?php echo ($vo["id"]); ?>" class="confirm" data-role="您确定要删除吗？删除后数据将不能恢复！">删除</a>
                        </li>
