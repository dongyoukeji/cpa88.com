...
<div class="admin_r_all" style="height: 480px;">
    <div class="admin_bg_all">
        ...
        <div class="admin_bg_t">
            <a href="#" id="admin_bg_t_x">结算列表</a>
            <div class="admin_bg_t_ss">
                <form action="/Admin/Balance">
                    <span>
                        <input name="username" value="<?php echo ($search["name"]); ?>" type="text" placeholder="用户名">
                        <input name="" type="submit" value="搜索">
                        <input type="button" class="btn-all" data-role="/Admin/Balance/index" value="所有">
                    </span>
                    <strong>
                        <select name="status">
                            <option value="-1">状态</option>
                            <option value="1" <?php if(($search["status"]) == "1"): ?>selected<?php endif; ?>>已结算</option>
                            <option value="0" <?php if(($search["status"]) == "0"): ?>selected<?php endif; ?>>未结算</option>
                        </select>
                    </strong>
                </form>
            </div>
        ...
        <div class="admin_bg_b">
            <ul class="admin_bg_b_t">
                ...
                <li class="admin_bg9">用户/类别/联系人</li>
                <li class="admin_bg6">产品</li>
                <li class="admin_bg2">类别</li>
                <li class="admin_bg2">推广</li>
                <li class="admin_bg4">包名</li>
                <li class="admin_bg2">状态</li>
                <li class="admin_bg2">操作</li>
            </ul>
            <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul>
                        <li class="admin_bg5"><?php echo (date('Y-m-d',$vo["time"])); ?></li>
                        <li class="admin_bg9"><?php echo (get_member($vo["mid"])); ?></li>
                        <li class="admin_bg6"><?php echo (get_pack_name($vo["pid"])); ?></li>
                        <li class="admin_bg2"><?php echo (get_pack_t($vo["pid"])); ?></li>
                        <li class="admin_bg2"><?php echo (get_pack_type($vo["pid"])); ?></li>
                        <li class="admin_bg4"><?php echo (get_pro_name($vo["pid"])); ?></li>
                        <li class="admin_bg2">
                           <?php if(($vo["status"]) == "0"): ?><font id="admin_color2">未结算</font>
                               <?php else: ?>
                               <font id="admin_color1">已结算</font><?php endif; ?>
                        </li>
                        <li class="admin_bg2">
                            <a href="/Admin/Balance/check?id=<?php echo ($vo["id"]); ?>&mid=<?php echo ($vo["mid"]); ?>">查看</a>
                        </li>
